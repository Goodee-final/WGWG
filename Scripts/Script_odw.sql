...
SELECT *
			FROM (
				SELECT e.*,ROW_NUMBER () OVER(ORDER BY EMP_NO DESC) RNUM
					FROM EMP e WHERE EMP_NO LIKE LOWER('%12%')
...
SELECT *
			FROM (
				SELECT e.*,ROW_NUMBER () OVER(ORDER BY EMP_NO DESC) RNUM
					FROM EMP e WHERE EMP_NM LIKE LOWER('%이%')
				)WHERE RNUM BETWEEN 1 AND 5;

			
			
--사원 상세조회
SELECT e.EMP_NO,e.EMP_NM ,e.EMAIL ,e.PHOTO ,e.ADDRESS ,e.TEL ,e.HIREDATE ,P.POSITION_NM ,d.DEPT_NM,e.WORK_ST 
   FROM EMP e, DEPARTMENT d, "POSITION" p 
   WHERE e.DEPT_NO=d.DEPT_NO AND e.POSITION_NO = p.POSITION_NO AND EMP_NO =#{emp_no};
  
SELECT e.EMP_NO,e.EMP_NM ,e.EMAIL ,e.PHOTO ,e.ADDRESS ,e.TEL ,e.HIREDATE ,P.POSITION_NM AS POSITION_NM,d.DEPT_NM AS DEPT_NM ,e.WORK_ST 
   FROM EMP e, DEPARTMENT d, "POSITION" p 
   WHERE e.DEPT_NO=d.DEPT_NO AND e.POSITION_NO = p.POSITION_NO AND EMP_NO =1;
