...
public class ChatSocketHandler extends TextWebSocketHandler {
	...
	// 채팅에 참여하는 webSocket session값을 담은 리스트
	// 채팅의 내용을 보내는 대상자
	private List<WebSocketSession> webSocketList = new ArrayList<>();
	...
	public ChatSocketHandler() {
		webSocketList = new ArrayList<WebSocketSession>();
	}
	...
	public void afterConnectionEstablished(WebSocketSession session) throws Exception {
		...
		logger.info("웹소켓에 현재 참여하고 있는 SessionID : " + session.getId() + " // 참여 인원 : " + webSocketList.size());
...
//		for (WebSocketSession wss : webSocketList) {
//			TextMessage msg = new TextMessage(message.getPayload());
//			wss.sendMessage(msg);
//		}

		logger.info("웹소켓의 모든 참여자들에게 메시지 전달 handleTextMessage");
		logger.info("handleTextMessage 사용자가 메세지를 보냈을 때 호출되는 메소드");
		...
		if (msg != null && !msg.equals("")) { // 대화 내용이 전달됨
			...
			// 머리말에 emp_no 이 포함되어 있다면 map에 (websocketSession, 전달받은 nickName)을 넣어줌
			// ex) emp_no 스폰지밥 이라면 => 스폰지밥 만 추출해서 => map.put(session, "스폰지밥")
			if (msg.indexOf("emp_no") != -1) {
				// indexOf는 확인용임!! 이걸로 글 짜를 생각하지 마!!!
				// 쪼개는 건 무조건 split!!
				map.put(session, msg.replace("mem_no", ""));
				logger.info("방금 참여한 자신의 mem_no Map 값 : {}", map.get(session));

				// 입장 메시지를 보내줌
				for (WebSocketSession s : webSocketList) {
					// 현재 자신은 빼고 보내고 싶다 or 그룹 채팅을 하고 싶다 인 경우
					// if(s != session) {}

					SimpleDateFormat sdf = new SimpleDateFormat("yyyy-mm-dd HH24:mm:ss");
					String out = sdf.format(new Date());
					s.sendMessage(new TextMessage("<font color='black' size='5px'>" + map.get(session)
							+ " 님이 입장하셨습니다.+(" + out + ")" + "</font>"));
	...
	public void afterConnectionClosed(WebSocketSession session, CloseStatus status) throws Exception {
		...
		for (WebSocketSession s : webSocketList) {
			// webSocketList에는 현재 참여중인 모든 참여자의 websocketSession이 담겨있음
			// s.sendMessage(new TextMessage(null)); 여기 null 대신 script 쓰면 그게 바로 실시간 알람!
			s.sendMessage(new TextMessage(
					"<font color='green' size='5px'>" + map.get(session) + " 님이 퇴장하셨습니다.+(" + out + ")" + "</font>"));
